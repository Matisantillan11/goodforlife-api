...
//	@inject(TYPES.Validable) @named(TYPES.Session) private sessionDto: Validable
	...
	private initializeRoutes(validationProvider: Validateable) {
		//this.router.get(`${this.path}/connections`, [], this.getConnections);
...
/* 	private getConnections = async (request: Request, response: Response, next: NextFunction) => {

		let socketList: Set<string> = await this.socket.io.allSockets()

		response.status(200).send({ connections: socketList.size })
						...
						const emailResponse = await this.sendMailController.sendMail(
							...
							`<!DOCTYPE html>
								...
								<head>
									...
									<title>iMexico Real Estate | Recupero de contraseña</title>
								<body>
									<a href=${`https://imexico-frontend.herokuapp.com/resetPass/${userResponse.result._id}`}>
									<div>
										<img src="https://ymp.ecosaludcentro.com/image/recuperarContrasena.jpg" alt="iMexico recupero de contraseña" style="width: 100%;"/>
									</div>
									</a>
								</body>
								</html>`,
							from,
							pass,
							'iMexico Real Estate | Recupero de contraseña'
						) */
						...
						const emailResponse = await this.sendMailController.sendMail(
							...
							`<!DOCTYPE html>
								...
								<head>
									<meta charset="UTF-8">
									<meta name="viewport" content="width=device-width, initial-scale=1.0">
									<title>Registro de usuario - Good For Life/title>
								<body>
									<a href=${`url/${registerResponse.result.user._id}`}>
										Validar email
									</a>
								</body>
								</html>`,
							from,
							pass,
							'Good For Life - Validación de cuenta'
						)
